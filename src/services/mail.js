        ...
        <tr>
          <td style="padding:20px 36px;">
            <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e2e8f0;border-radius:10px;overflow:hidden;border-collapse:collapse;">
              ...
              ${message ? `<tr><td style="padding:11px 16px;background:#f7fafc;font-size:13px;color:#718096;font-weight:600;vertical-align:top;">Reason</td><td style="padding:11px 16px;font-size:13px;color:#2d3748;line-height:1.6;">${message}</td></tr>` : ""}
            </table>
          </td>
        </tr>
        <tr>
          <td style="padding:0 36px 28px;">
            <div style="background:#fffbeb;border:1px solid #f6e05e;border-radius:10px;padding:14px 18px;">
              <p style="margin:0;font-size:13px;color:#744210;line-height:1.6;">⚠️ Please log in to <strong>Task Manager</strong> and go to <strong>Documents → Access Requests</strong> to approve or deny this request.</p>
            </div>
          </td>
        </tr>
        <tr>
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
        ...
        <tr>
          <td style="padding:0 36px 16px;">
            <p style="margin:0;font-size:12px;color:#a0aec0;text-align:center;">🔗 This link is valid for <strong>7 days</strong>. After that, please log in directly.</p>
          </td>
        </tr>` : ""}
        <tr>
          <td style="padding:0 36px 32px;" align="center">
            <a href="${ctaHref}" style="display:inline-block;background:${gradient};color:#ffffff;font-size:14px;font-weight:600;padding:12px 32px;border-radius:8px;text-decoration:none;">${ctaLabel}</a>
          </td>
        </tr>
        <tr>
          <td style="background:#f7fafc;padding:18px 36px;border-top:1px solid #e2e8f0;">
...
// ─── NEW: WELCOME STAFF MAIL ──────────────────────────────────────────────────
// Sent when an admin/owner adds a new staff member manually
export const sendWelcomeStaffMail = async ({ name, email, password, companyName }) => {
  const transporter = createTransporter();
...
<body style="margin:0;padding:0;background:#f4f6fb;font-family:'Segoe UI',Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f4f6fb;padding:32px 0;">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
        <tr>
          <td style="background:linear-gradient(135deg,#924485 0%,#931678 100%);padding:32px 36px 28px;">
            <p style="margin:0 0 10px;font-size:28px;">🎉</p>
            <h1 style="margin:0;color:#ffffff;font-size:22px;font-weight:700;">Welcome to ${companyName}!</h1>
            <p style="margin:6px 0 0;color:rgba(255,255,255,0.8);font-size:14px;">Your account has been created successfully.</p>
          </td>
        </tr>
        <tr>
          <td style="padding:28px 36px 0;">
            <p style="margin:0 0 16px;font-size:15px;color:#1e293b;font-weight:600;">Hi ${name},</p>
            <p style="margin:0;font-size:14px;color:#475569;line-height:1.7;">You've been added as a team member on <strong>${companyName}'s</strong> task management platform. Here are your login details:</p>
          </td>
        </tr>
        <tr>
          <td style="padding:20px 36px;">
            <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e2d0ef;border-radius:10px;overflow:hidden;border-collapse:collapse;background:#f8f4fb;">
              <tr>
                <td style="padding:11px 16px;font-size:13px;color:#718096;font-weight:600;width:36%;border-bottom:1px solid #e2d0ef;">Email</td>
                <td style="padding:11px 16px;font-size:13px;color:#1e293b;font-weight:700;font-family:monospace;border-bottom:1px solid #e2d0ef;">${email}</td>
              </tr>
              <tr>
                <td style="padding:11px 16px;font-size:13px;color:#718096;font-weight:600;border-bottom:1px solid #e2d0ef;">Password</td>
                <td style="padding:11px 16px;font-size:13px;color:#1e293b;font-weight:700;font-family:monospace;border-bottom:1px solid #e2d0ef;">${password}</td>
              </tr>
              <tr>
                <td style="padding:11px 16px;font-size:13px;color:#718096;font-weight:600;">Company</td>
                <td style="padding:11px 16px;font-size:13px;color:#1e293b;font-weight:700;">${companyName}</td>
              </tr>
            </table>
          </td>
        </tr>
        <tr>
          <td style="padding:0 36px 16px;">
            <div style="background:#fffbeb;border:1px solid #fcd34d;border-radius:8px;padding:12px 16px;">
              <p style="margin:0;font-size:13px;color:#92400e;">⚠️ Please change your password after your first login.</p>
            </div>
          </td>
        </tr>
        <tr>
          <td style="padding:0 36px 32px;" align="center">
            <a href="${loginUrl}" style="display:inline-block;background:linear-gradient(135deg,#924485,#931678);color:#ffffff;font-size:14px;font-weight:600;padding:12px 32px;border-radius:8px;text-decoration:none;">Login to Your Account →</a>
          </td>
        </tr>
        <tr>
          <td style="background:#f7fafc;padding:18px 36px;border-top:1px solid #e2e8f0;">
            <p style="margin:0;font-size:12px;color:#a0aec0;text-align:center;">This is an automated message from <strong>${companyName}</strong> Task App. Do not reply.</p>
          </td>
        </tr>
      </table>
    </td></tr>
  ...
  console.log("✅ Welcome staff email sent to:", email);
};

// ─── NEW: COMPANY CREATED MAIL ────────────────────────────────────────────────
// Sent to the owner right after they sign up and create their company
export const sendCompanyCreatedMail = async ({ ownerName, companyName, email, password }) => {
  const transporter = createTransporter();
  const loginUrl = process.env.FRONTEND_URL || "http://localhost:5173";
  ...
  await transporter.sendMail({
    from: `"TaskApp" <${process.env.EMAIL_USER}>`,
    to: email,
    subject: `Your Company "${companyName}" is Live on TaskApp 🚀`,
    html: `
<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"/></head>
<body style="margin:0;padding:0;background:#f4f6fb;font-family:'Segoe UI',Arial,sans-serif;">
    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
        <tr>
          <td style="background:linear-gradient(135deg,#924485 0%,#931678 100%);padding:32px 36px 28px;">
            <p style="margin:0 0 10px;font-size:28px;">🚀</p>
            <h1 style="margin:0;color:#ffffff;font-size:22px;font-weight:700;">${companyName} is Live!</h1>
            <p style="margin:6px 0 0;color:rgba(255,255,255,0.8);font-size:14px;">Your company workspace has been created.</p>
            <span style="display:inline-block;margin-top:10px;background:rgba(255,255,255,0.2);color:#fff;font-size:12px;font-weight:600;padding:4px 14px;border-radius:20px;text-transform:uppercase;letter-spacing:1px;">Company Owner</span>
          </td>
        </tr>
        <tr>
          <td style="padding:28px 36px 0;">
            <p style="margin:0 0 14px;font-size:15px;color:#1e293b;font-weight:600;">Congratulations, ${ownerName}! 🎉</p>
            <p style="margin:0;font-size:14px;color:#475569;line-height:1.7;">Your company <strong>${companyName}</strong> has been registered on TaskApp. You are the owner of this workspace.</p>
          </td>
        </tr>
        <tr>
          <td style="padding:20px 36px;">
            <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e2d0ef;border-radius:10px;overflow:hidden;border-collapse:collapse;background:#f8f4fb;">
              ...
              <tr>
                <td style="padding:11px 16px;font-size:13px;color:#718096;font-weight:600;">Role</td>
                <td style="padding:11px 16px;font-size:13px;color:#1e293b;font-weight:700;">Company Owner (Admin)</td>
              </tr>
            </table>
          </td>
        </tr>
        <tr>
          <td style="padding:0 36px 16px;">
            <p style="margin:0 0 10px;font-size:13px;color:#924485;font-weight:700;text-transform:uppercase;letter-spacing:0.8px;">🗺️ Next Steps</p>
            <table cellpadding="0" cellspacing="6">
              ${["Log in and change your password","Add team members from the Staff section","Create your first project and assign tasks","Configure company working hours in Settings"]
                .map((step, i) => `<tr>
                  <td style="vertical-align:top;padding-right:10px;">
                    <span style="display:inline-block;background:#924485;color:#fff;width:20px;height:20px;border-radius:50%;font-size:11px;font-weight:700;text-align:center;line-height:20px;">${i+1}</span>
                  </td>
                  <td style="font-size:13px;color:#475569;line-height:1.6;">${step}</td>
                </tr>`).join("")}
            </table>
          </td>
        </tr>
        <tr>
          <td style="padding:16px 36px 32px;" align="center">
            <a href="${loginUrl}" style="display:inline-block;background:linear-gradient(135deg,#924485,#931678);color:#ffffff;font-size:14px;font-weight:600;padding:12px 32px;border-radius:8px;text-decoration:none;">Go to Dashboard →</a>
          </td>
        </tr>
        <tr>
          <td style="background:#f7fafc;padding:18px 36px;border-top:1px solid #e2e8f0;">
            <p style="margin:0;font-size:12px;color:#a0aec0;text-align:center;">This is an automated message from <strong>TaskApp</strong>. Do not reply.</p>
          </td>
        </tr>
      </table>
    </td></tr>
  ...
  console.log("✅ Company created email sent to:", email);
};