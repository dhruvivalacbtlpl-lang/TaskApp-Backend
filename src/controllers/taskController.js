...
export const createTask = async (req, res) => {
  try {
    ...
    await logAudit(req, "Task", "CREATE",
      `Created task "${name}"`,
      { entityId: task._id.toString(), entityName: name }
    );
...
export const updateTask = async (req, res) => {
  try {
    ...
    await logAudit(req, "Task", "UPDATE",
      `Updated task "${task.name}"`,
      { entityId: task._id.toString(), entityName: task.name }
    );
    res.json(task);
...
export const deleteTask = async (req, res) => {
  try {
    ...
    await logAudit(req, "Task", "DELETE",
...
export const createIssue = async (req, res) => {
  try {
    ...
    await logAudit(req, "Issue", "CREATE",
      `Created issue "${name}"`,
      { entityId: issue._id.toString(), entityName: name }
    );
    res.status(201).json(populated);
...
export const updateIssue = async (req, res) => {
  try {
    ...
    await logAudit(req, "Issue", "UPDATE",
      `Updated issue "${issue.name}"`,
      { entityId: issue._id.toString(), entityName: issue.name }
    );
    res.json(issue);
